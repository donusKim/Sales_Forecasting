...
def makeModel(filterN,layerN,dr,timesteps):
  ...
  encode_features = concatenate([ yearAgo_in, quarterAgo_in], axis=2)
  ...
  c1 = Conv1D(filterN, 3, dilation_rate=1, padding='causal', activation='relu')(x_in)
  conv_out = Dropout(dr)(c1)
  conv_out = Flatten()(conv_out)
  dnn_out = Dense(layerN, activation='relu')(Flatten()(seq_in))
  dnn_out = Dropout(dr)(dnn_out)
  x = concatenate([conv_out, dnn_out])
  x = Dense(layerN, activation='relu')(x)
  x = Dropout(dr)(x)
  output = Dense(4, activation='relu')(x)
  model = Model([seq_in, temp_in,rain_in,trend_in, yearAgo_in, quarterAgo_in, brand_mean_in, point_mean_in], output)
  return model
